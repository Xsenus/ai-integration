...
## Точность нормализации score (обновление)
* Нормализация внутренних score в `analyze_json._normalize_score()` переведена с округления до 2 знаков на 4 знака после запятой.
* Причина: при 2 знаках значение в диапазоне `0..1` после умножения на 100 почти всегда отображается как `xx.0%` на фронте.
* При 4 знаках сохраняется дробная часть (`81.5%`, `73.4%` и т.п.), что лучше отражает реальную разницу между кандидатами и источниками score.
* Важно: диапазон и логика clamp не менялись — по-прежнему `0.0 <= score <= 1.0`, а значения `1..100` трактуются как проценты и делятся на 100.
