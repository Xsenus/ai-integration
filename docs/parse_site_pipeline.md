...
## 2.1. Приоритет источников доменов (новое правило)
* Домены из явных источников (`web_sites`, `clients_requests`, `ib_clients`, ручные `parse_domain/parse_domains`) считаются приоритетными.
* Домены, извлечённые из e-mail (`dadata_result.emails`, `parse_emails`), используются только как **fallback**, если явных доменов не найдено.
* Это сделано, чтобы снизить риск попадания внешних/партнёрских сайтов в PRIMARY SOURCE и не смешивать нерелевантный контент в downstream-этапах (`analyze-json`, `ib-match`, `equipment-selection`).
* В логах явно фиксируется, когда e-mail домены были найдены, но намеренно пропущены из-за наличия явных доменов.
...
| Поле | Описание |
| --- | --- |
| `status` | `success`, `partial_success` или `fallback` — в зависимости от результата парсинга и наличия фолбэка по ОКВЭД. |
...
* До начала обхода доменов (при `save_client_request=true`) сервис выполняет предварительный upsert в `clients_requests`, чтобы `company_id` существовал даже если сайт недоступен и в `pars_site` не появилось ни одного чанка.
* Если доменов нет или все попытки парсинга завершились ошибкой, сервис использует fallback: отправляет основной ОКВЭД во внешний маршрут `/v1/prompts/site-unavailable`, получает `PRODCLASS` по ОКВЭД и возвращает ответ со статусом `fallback` и полем `site_unavailable`.
