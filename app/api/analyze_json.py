...
_AI_OPENAI_TABLE_ENSURED = False
_AI_REQUEST_COSTS_TABLE_ENSURED = False
...
async def _ensure_ai_site_openai_responses(conn: AsyncConnection) -> None:
    """Создаёт таблицу ai_site_openai_responses в Postgres при её отсутствии."""

    global _AI_OPENAI_TABLE_ENSURED
    if _AI_OPENAI_TABLE_ENSURED:
        return

    await conn.execute(
        text(
            """
            CREATE TABLE IF NOT EXISTS public.ai_site_openai_responses (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                text_pars_id BIGINT NOT NULL REFERENCES public.pars_site(id) ON UPDATE CASCADE ON DELETE CASCADE,
                company_id BIGINT REFERENCES public.clients_requests(id) ON UPDATE CASCADE ON DELETE CASCADE,
                domain TEXT,
                url TEXT,
                description TEXT,
    ...
    await conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS ix_ai_site_openai_responses_text_pars_id "
            "ON public.ai_site_openai_responses(text_pars_id)"
        )
    )
    await conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS ix_ai_site_openai_responses_company_id "
...
async def _ensure_ai_site_request_costs(conn: AsyncConnection) -> None:
    """Создаёт таблицу ai_site_request_costs для хранения стоимости запросов."""

    global _AI_REQUEST_COSTS_TABLE_ENSURED
    if _AI_REQUEST_COSTS_TABLE_ENSURED:
        return

    await conn.execute(
        text(
            """
            CREATE TABLE IF NOT EXISTS public.ai_site_request_costs (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                company_id BIGINT REFERENCES public.clients_requests(id) ON UPDATE CASCADE ON DELETE SET NULL,
                text_pars_id BIGINT REFERENCES public.pars_site(id) ON UPDATE CASCADE ON DELETE SET NULL,
                model TEXT,
                tokens_input INT,
                ...
                spend_month_to_date_usd NUMERIC(12,6),
    ...
    await conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS ix_ai_site_request_costs_company_id "
            "ON public.ai_site_request_costs(company_id)"
        )
    )
    await conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS ix_ai_site_request_costs_text_pars_id "
            "ON public.ai_site_request_costs(text_pars_id)"
        )
    )

    _AI_REQUEST_COSTS_TABLE_ENSURED = True
...
def _extract_billing_summary(payload: Any) -> dict[str, Any]:
    """Извлекает сводку баланса billing_summary из ответа внешнего сервиса."""

    if not isinstance(payload, Mapping):
        return {}

    billing_summary = payload.get("billing_summary")
    if not isinstance(billing_summary, Mapping):
        return {}
    ...
    *,
    ...
    async with engine.begin() as conn:
        ...
        params: dict[str, Any] = {
            ...
            # WHY: для fallback по ОКВЭД нет достоверной оценки уверенности от анализа сайта.
            # 1.0 здесь создаёт ложный сигнал "100%" и ломает downstream-выбор.
        ...
        # Data hygiene: мягко нормализуем исторические fallback-записи.
        # Не удаляем историю, но обнуляем legacy score=1 для okved://fallback,
        # чтобы прошлые прогоны не "побеждали" новые данные.
        await conn.execute(
            text(
                """
                UPDATE public.ai_site_prodclass AS ap
                SET
                    prodclass_score = NULL,
                    okved_score = NULL,
                    description_okved_score = NULL
                FROM public.pars_site AS pst
                WHERE pst.id = ap.text_pars_id
                  AND pst.company_id = :company_id
                  AND pst.url = 'okved://fallback'
                  AND (
                      ap.prodclass_score = 1
                      OR ap.okved_score = 1
                      OR ap.description_okved_score = 1
                  )
                """
            ),
...
async def _apply_db_payload(
    ...
    billing_summary: Optional[Mapping[str, Any]] = None,
    ...
    async with engine.begin() as conn:
        ...
        request_cost_payload = {
            ...
            "text_pars_id": snapshot.pars_id,
            "model": request_cost.get("model") if request_cost else None,
            "tokens_input": request_cost.get("input_tokens") if request_cost else None,
            "tokens_cached_input": request_cost.get("cached_input_tokens") if request_cost else None,
            "tokens_output": request_cost.get("output_tokens") if request_cost else None,
            "tokens_total": request_cost.get("tokens_total") if request_cost else None,
            "cost_usd": request_cost.get("cost_usd") if request_cost else None,
            "remaining_usd": billing_summary.get("remaining_usd") if billing_summary else None,
            "spend_month_to_date_usd": (
                billing_summary.get("spend_month_to_date_usd") if billing_summary else None
            ),
            "limit_usd": billing_summary.get("limit_usd") if billing_summary else None,
        }
        ...
        try:
            await _ensure_ai_site_openai_responses(conn)
            await conn.execute(insert_openai_sql, openai_payload)
        except Exception as exc:  # noqa: BLE001
            message = str(exc)
            message_lower = message.lower()
            missing_table = (
                "ai_site_openai_responses" in message_lower
                and (
                    "does not exist" in message_lower
                    or "undefinedtable" in message_lower
                    or "undefined_table" in message_lower
                )
            )
            if missing_table:
                log.warning(
                    "analyze-json: ai_site_openai_responses missing — creating table before retry (pars_id=%s)",
                    snapshot.pars_id,
                )
                global _AI_OPENAI_TABLE_ENSURED
                _AI_OPENAI_TABLE_ENSURED = False
                ...
                try:
                    ...
                    log.warning(
                        "analyze-json: retry failed to store full openai response (pars_id=%s): %s",
                        snapshot.pars_id,
                        retry_exc,
                    )
            else:
        ...
        if request_cost or billing_summary:
            try:
                await _ensure_ai_site_request_costs(conn)
                await conn.execute(
                    text(
                        """
                        INSERT INTO public.ai_site_request_costs (
                            company_id, text_pars_id, model,
                            tokens_input, tokens_cached_input, tokens_output, tokens_total,
                            cost_usd, remaining_usd, spend_month_to_date_usd, limit_usd,
                            created_at
                        )
                        VALUES (
                            :company_id, :text_pars_id, :model,
                ...
                log.warning(
                    "analyze-json: failed to store request cost (pars_id=%s): %s",
                    snapshot.pars_id,
                    exc,
                )
...
async def _run_analyze(
    ...
    *,
    ...
    for idx, snapshot in enumerate(snapshots, start=1):
        ...
        billing_summary = _extract_billing_summary(response_json)
