...
async def pars_site_update_vector(
    *,
    ...
    sql = text(
        ...
        SET text_vector = CASE WHEN :vec IS NULL THEN NULL ELSE CAST((:vec)::text AS vector) END
        FROM latest
        WHERE ps.company_id = :company_id
...
async def ensure_parsing_schema() -> None:
    ...
    try:
        async with eng.begin() as conn:
            ...
            # 5) ai_site_openai_responses — полные ответы анализа по домену/компании
            await conn.execute(text("""
            ...
            # Сохраняем 4 знака после запятой для новых и существующих инсталляций.
            await conn.execute(
                text(
                    """
            ...
            await conn.execute(
                text(
                    """
                    ALTER TABLE public.ai_site_prodclass
                    ALTER COLUMN prodclass_score TYPE NUMERIC(6,4)
                    """
                )
            )
            ...
            await conn.execute(
                text(
                    """
                    ALTER TABLE public.ai_site_goods_types
                    ALTER COLUMN goods_types_score TYPE NUMERIC(6,4)
                    """
                )
            )
            await conn.execute(
                text(
                    """
                    ALTER TABLE public.ai_site_equipment
                    ALTER COLUMN equipment_score TYPE NUMERIC(6,4)
                    """
                )
            )
